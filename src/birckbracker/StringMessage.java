package birckbracker;

public class StringMessage
{
    public final String Font  = "serif";
    public final String MensagemStart  = "Pressione enter para começar";
    public final String MensagemRestart  = "Pressione Enter para recomeçar.";
    public final String MensagemScore  = "Pontos: ";
    public final String MensagemWin = "Parabéns, você ganhou! Pontos: ";
    public final String MensagemLose = "Fim de jogo! Pontos: ";
}
